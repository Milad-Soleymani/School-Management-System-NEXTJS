import React from "react";
import AttendanceChart from "./AttendanceChart";
import Image from "next/image";
import prisma from "@/lib/prisma";

// Server Component
const AttendanceChartContainer = async () => {
  const today = new Date();
  const dayOfWeek = today.getDay();
  const daysSinceSaturday = (dayOfWeek + 1) % 7; // شنبه=0
  const lastSaturday = new Date(today);
  lastSaturday.setDate(today.getDate() - daysSinceSaturday);

  const resData = await prisma.attendance.findMany({
    where: {
      date: {
        gte: lastSaturday,
      },
    },
    select: {
      date: true,
      present: true,
    },
  });

  // روزهای هفته ایران (تعطیلی پنج‌شنبه و جمعه)
  const daysOfWeek = ["شنبه", "یک‌شنبه", "دوشنبه", "سه‌شنبه", "چهارشنبه"];

  const attendanceMap: { [key: string]: { present: number; absent: number } } =
    {};
  daysOfWeek.forEach((day) => (attendanceMap[day] = { present: 0, absent: 0 }));

  resData.forEach((item) => {
    const itemDate = new Date(item.date);
    const itemDay = (itemDate.getDay() + 1) % 7; // شنبه=0
    if (itemDay < 5) {
      const dayName = daysOfWeek[itemDay];
      if (item.present) attendanceMap[dayName].present += 1;
      else attendanceMap[dayName].absent += 1;
    }
  });

  const data = daysOfWeek.map((day) => ({
    name: day,
    حاضر: attendanceMap[day].present,
    غایب: attendanceMap[day].absent,
  }));

  return (
    <div className="bg-white rounded-lg p-4 h-full">
      {/* هدر نمودار */}
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-lg font-semibold">حضور و غیاب</h1>
        <Image src="/moreDark.png" alt="بیشتر" width={20} height={20} />
      </div>
      <AttendanceChart data={data} />
    </div>
  );
};

export default AttendanceChartContainer;
