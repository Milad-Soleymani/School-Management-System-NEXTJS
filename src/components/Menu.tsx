...
const menuItems = [
  {
    title: "منو",
    items: [
      { icon: "/home.png", label: "خانه", href: "/", visible: ["admin", "teacher", "student", "parent"] },
      { icon: "/teacher.png", label: "معلّم ها", href: "/list/teachers", visible: ["admin", "teacher"] },
      { icon: "/student.png", label: "دانش اموزان", href: "/list/students", visible: ["admin", "teacher"] },
  ...
  return (
    <div className="flex flex-col items-end font-thin w-full text-[13px]">
      {menuItems.map(section => (
        <div key={section.title} className="flex flex-col p-0 w-full items-center">
          {/* عنوان بخش منو */}
          <span className="hidden lg:block text-gray-400 font-semibold my-4">{section.title}</span>
          ...
          {section.items
            ...
            .map(item => (
              <Link
                href={item.href}
                key={item.href}
                className="flex items-center justify-center w-[100%] lg:justify-end gap-4 text-gray-500 py-1.5 rounded-md hover:bg-blueSkyLight"
              >
